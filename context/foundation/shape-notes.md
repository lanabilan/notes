...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: pain category
      decision: workflow friction — practice interrupted by ads/paywalls
    - topic: insight
      decision: no good free versions exist that just let you practice
    - topic: primary persona
      decision: me — first and primary user for v1
    - topic: auth strategy
      decision: guests practice with no login; optional OAuth in v1 for saved progress; flat roles
    - topic: wrong-answer feedback
      decision: do not auto-highlight correct key; learner taps a button to reveal highlight and note name
    - topic: delivery surface
      decision: mobile-first web
    - topic: progress persistence
      decision: progress stored only for OAuth users in their profiles; guests are session-only
  frs_drafted: 11
---

# Shape notes

Seed (from `MVP.md`): a single-player app to learn piano notes — see a note on the staff and find it on the piano.
...
- FR-007: Guest can practice without signing in; guest progress is session-only and is not saved. Priority: must-have
  > Socrates: Counter-argument considered: open guest access invites abuse. Resolution: no counter-argument; it stands as written — no login wall is a guardrail. Revised: guests do not get persistent progress.
...
- FR-010: App is mobile-first — layout adapts to mobile devices with large tap targets, readable staff, usable on phone without horizontal scroll or tiny keys. Priority: must-have
  > Socrates: Counter-argument considered: mobile-first adds significant UI work. Resolution: no counter-argument; it stands as written — primary use case is phone practice. Confirmed: mobile-first.
...
1. **Solfège vs letter names** — default letter names; language toggle later.
2. **Full songs / rhythm / two-hand pieces** — not discussed; assumed out of v1 scope per MVP.md unless added later.
3. **PWA installability** — product is mobile-first web; installable PWA vs mobile web page is a stack/delivery choice.
